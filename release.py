...
def export_ufo(path):
    ...
    info.versionMajor, info.versionMinor = 0, 402
...
def preview():
    ...
    doc = '''<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Damonic 0.4.2 Alpha</title><style>
    FACES
...
def package():
    ...
    # Frozen proof fonts are source inputs, not installation choices.
    binary=[p for p in binary if not (p.relative_to(ROOT).parts[0]=='docs' and p.suffix in ('.ttf','.ttc','.woff2'))]
